...
<div class="container">
    <div class="mb-6">
        <h2 class="page-title">Ajouter une Réparation</h2>
        <p class="page-subtitle">Remplissez le formulaire ci-dessous pour ajouter une nouvelle réparation.</p>
    </div>

    <div class="card">
        <form action="{{ route('reparations.store') }}" method="POST" class="card-body">
            @csrf
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select name="vehicule_id" id="vehicule_id" required
                        ...
                        @foreach($vehicules as $vehicule)
                            <option value="{{ $vehicule->id }}" {{ (old('vehicule_id', request('vehicule_id')) == $vehicule->id) ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="technicien_id" id="technicien_id" required
                        ...
                        @foreach($techniciens as $technicien)
                            <option value="{{ $technicien->id }}" {{ (old('technicien_id', request('technicien_id')) == $technicien->id) ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="duree_main_oeuvre" id="duree_main_oeuvre" value="{{ old('duree_main_oeuvre') }}" min="1" required
                ...
                <div class="form-group form-grid-full">
                    ...
                    <textarea name="objet_reparation" id="objet_reparation" rows="4" required
                        class="form-textarea @error('objet_reparation') error @enderror">{{ old('objet_reparation') }}</textarea>
